...
with main_container:
    if st.checkbox("Change main folder") or st.session_state.dir_warning:
        with st.form("dir_form"):
            ...
            imagereward_check = st.checkbox("Calculate ImageReward score for each image?")
    ...
    if filter_by_dir:
        ...
        st.session_state["df"]["filename"] = st.session_state["df"]["filename"].apply(Path)

        # Get the unique parent directories of the files
        directories = st.session_state["df"]["filename"].apply(lambda x: x.parent).unique()
        ...
        if selected_dir:
            ...
            st.session_state["df"] = helper.load_data(selected_dir, cleanup=False, imagereward=st.session_state.imagereward_check)
