...
def load_data(directory, cleanup, imagereward=False):
    ...
    if directory == None:
        ...
        df = pvision.process_directory(directory=directory, cleanup=cleanup, imagereward=imagereward)
    return df

def set_directory(cleanup=False, imagereward=False):
    st.session_state["df"] = load_data(st.session_state["directory"], cleanup=cleanup, imagereward=imagereward)

def reset_cached_images():
    st.cache_resource.clear()
    st.cache_data.clear()
    st.session_state["df"] = load_data(st.session_state["directory"], cleanup=True, imagereward=st.session_state.imagereward_check)

def store_metadata_in_session(index):
    # Store the image and its metadata in the session state
    st.session_state["filename"] = st.session_state["df"].loc[
        index, "filename"
    ]
    st.session_state["width"] = st.session_state["df"].loc[index, "width"]
    st.session_state["height"] = st.session_state["df"].loc[
        index, "height"
    ]
    ...
    st.session_state["metadata"] = st.session_state["df"].loc[
        index, "metadata"
    ]
    st.session_state["imghash"] = st.session_state["df"].loc[
        index, "imghash"
    ]
